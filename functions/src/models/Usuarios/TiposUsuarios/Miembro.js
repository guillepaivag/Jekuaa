...
class Miembro {
    ...
    */
    ...
    static async actualizarBlog (uid, datosBlog, rutaArchivoTemp) {
        ...
        async function verificacion(uid) {
            const datosActualizados = {}
            const blog = new Blog()
            await blog.importarDatosBlogPorUID(uid)
            const mensajesError = JSON.parse( JSON.stringify( blog.mensajesError ) )

            // Verificar si tiene contenido
            const contenido = await Blog.obtenerContenido(uid, 'html')
            ...
            if (!soloTexto.length && !mensajesError.includes('Falta contenido para el blog.')) {
            ...
            await Blog.actualizarBlog( uid, datosActualizados )
        }
        verificacion(uid)
