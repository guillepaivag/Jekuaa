...
.onCreate(async ( change, context ) => {
    ...
    if (!doc.exists) await ref.set({ cantidad: 1 })
    else ref.update({ cantidad: incrementar })
...
.onDelete(async ( change, context ) => {
    ...
    ref.update({ cantidad: decrementar })
    ...
    if (docNuevo.data().seccion !== docViejo.data().seccion) {
        if (!docNuevo.data().seccion) {
            ...
            if (!doc.exists) {
                ...
                ref
                .collection('ContadoresBlogSeccion').doc(docNuevo.data().seccion)
                .update({ cantidad: incrementar })
            }
        }

        if (!docViejo.data().seccion) {
            ref.update({ cantidadSinSeccion: decrementar })
        } else {
            ref
            .collection('ContadoresBlogSeccion').doc(docViejo.data().seccion)
            .update({ cantidad: decrementar })
        }
    }
