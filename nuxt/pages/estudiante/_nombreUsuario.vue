...
export default {
    ...
    async asyncData({isDev, route, $axios, $firebase, store, env, params, query, req, res, redirect, error}) {
        ...
        try {
            ...
            let response = {}
            response.datosUsuario = await $axios.$get(`/api/usuario/datosUsuario/nombreUsuario/${nombreUsuario}`, config)
            response.authUsuario = await $axios.$get(`/api/usuario/datosAuthentication/nombreUsuario/${nombreUsuario}`, config)
            response.informacionUsuario = await $axios.$get(`/api/usuario/informacionUsuario/nombreUsuario/${nombreUsuario}`, config)
            response.ultimosBlogs = await $axios.$get(`/api/blog/ultimosBlogsPorPublicador/${response.datosUsuario.resultado.uid}`, body, config)
